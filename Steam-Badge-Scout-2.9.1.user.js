...
// @version      2.9.1
// @description  Nonstop Badge Scout mit Live-Fresh-Hunt: blendet auf Wunsch bereits begonnene Sammlungen mit aktiven Kaufaufträgen, Kartenbesitz oder Badge-Fortschritt aus und stellt sie beim Abschalten sofort wieder her. Dazu Marktampel, Sell-Supply, Buy/Sell-Deepchecks, Badge-/Foil-Optik und Themen-Suche. Keine Käufe oder Gebote.
...
S.activeBuyOrdersLoadedAt>0&&
['ready','loading'].includes(S.activeBuyOrdersStatus);
const hasActiveOrders=orderSnapshotUsable&&orders.length>0;

const ownedCards=S.mode==='foil'
?(result?.foilBadgeState?.cards||[])
:(result?.cost?.cards||[]);
const ownedCardKinds=ownedCards.filter(card=>Number(card?.owned||0)>0).length;
const ownedCopies=ownedCards.reduce(
...
freshStatus.textContent=`⚠ Kaufaufträge unbekannt – Besitz und Badge-Fortschritt werden weiterhin gefiltert; reine Order-Sets bleiben sicherheitshalber sichtbar${S.activeBuyOrdersError?` · ${S.activeBuyOrdersError}`:''}`;
...
`💰 ${S.activeBuyOrders.length} Karten-Kaufaufträge in ${setCount} Steam-Set(s) · `+
...
<h2>🎯 Badge Scout 2.9.1 – Live Fresh Hunt + Nonstop + Marktampel</h2>
...
<br><br>
<b>2.9.1 Live Fresh Hunt:</b> Der Scout lädt deine aktiven Karten-Kaufaufträge direkt von Steam.
...
<label title="Nur tief geprüfte Sets mit guter Marktbreite und niedriger bis moderater Kaufkonkurrenz. Grundlage ist eine kleine Karten-Stichprobe; keine Fill-Garantie."><input id="filterLowBidTop" type="checkbox">nur 💎/🟢 Low-Bid-Top (Deepcheck)</label>
...
<button id="refreshBuyOrders" class="btnGray" type="button">↻ Kaufaufträge aktualisieren</button>
...
refreshBuyOrders.textContent='↻ Kaufaufträge werden geladen …';
await ensureActiveBuyOrders(true);
if(S.modal===overlay){
S.cardNodes=new Map();
syncVisibleResults(true);
