...
// @version      3.0.0
// @description  Zweistufiger Steam Badge Scout: globaler Billigmarkt-Snapshot statt tausender Einzelrequests, gezielte Exaktprüfungen, adaptive 429-Sperre, begonnene Sammlungen/Fresh Hunt, Effizienzsortierung nach Preis plus Kartenzahl und manuelle Low-Bid-Deepchecks. Keine Käufe oder Gebote.
...
// 3.0: Steam wird gebündelt und defensiv angesprochen. Ein HTTP 429 öffnet
...
// auf einmal. Nur die besten noch unvollständigen Sets werden anschließend
// einzeln exakt validiert. So bleibt die Suche unter Steam-Limits nutzbar.
...
cheapCatalogFallbackCandidates:350,
cheapCandidateLimit:1400,
...
S.activeBuyOrdersLoadedAt>0&&
['ready','loading','stale'].includes(S.activeBuyOrdersStatus);
...
Number(collection?.ownedUnique||0)
);
...
const hasCollectionProgress=Boolean(collection?.started);
...
status('ℹ Kein marktbestätigter Low-Bid-Kandidat offen. Erst „Top-Markt exakt prüfen“ nutzen oder weitere Treffer laden.');
return;
}
...
if(S.modal){S.cardNodes=new Map();syncVisibleResults(true);}
}
})();
...
return[
'yellow',
`🟡 Vorfilter · SCE-Schätzwert ${result.cost.sceFullSetText || '—'} (nicht live); Steam-Livepreis noch nicht vollständig bestätigt`
];
}
...
`SCE-Schätzwert (externer Vorfilter, nicht live): `+
...
?'Noch kein geprüfter/vorgemerkter Low-Bid-Top-Treffer. Nutze „Top-Markt exakt prüfen“ und danach „Top 5 Low-Bid prüfen“.'
:filterState().onlyMarketSuper||filterState().onlyGreen
?'Noch kein durch vollständige Steam-Livedaten bestätigter Treffer für diesen strengen Filter. SCE-Schätzwerte werden absichtlich nicht grün gefärbt; „Top-Markt exakt prüfen“ kann weitere Kandidaten bestätigen.'
...
// 3.0: Im normalen Billigmodus kommen Badge-Zusammenfassung und Marktzeilen
...
if(!S.stop&&!networkPaused&&S.nextCandidateIndex>=total&&S.mode==='cheap'){
...
status(`✅ Scout fertig: ${total} gezielt ausgewählte Kandidaten geprüft · ${S.results.length} Treffer geladen · ${S.stats.marketValidations} Top-Marktprüfungen · ${S.deepCheckCompleted} manuelle Low-Bid-Deepchecks.${limit?` ${limit}; weitere Exaktprüfung später möglich.`:''}`);
}
}
...
const snapshotApps=new Set(S.globalMarketRowsByApp.keys());
const selected=[];
const selectedIds=new Set();
const addCandidate=candidate=>{
const appid=String(candidate?.appid||'');
if(!appid||selectedIds.has(appid))return;
selectedIds.add(appid);
selected.push(candidate);
};

for(const candidate of withPrice){
if(snapshotApps.has(String(candidate.appid)))addCandidate(candidate);
if(selected.length>=CFG.cheapCandidateLimit)break;
}
for(const candidate of withPrice.slice(0,CFG.cheapCatalogFallbackCandidates)){
if(selected.length>=CFG.cheapCandidateLimit)break;
addCandidate(candidate);
}
for(const candidate of withPrice){
if(trackedApps.has(String(candidate.appid)))addCandidate(candidate);
}
selected.sort((a,b)=>
a.prelimEfficiency-b.prelimEfficiency||
a.cardsInSet-b.cardsInSet||
a.prelimCost-b.prelimCost
);
...
status(
`💸 Schnellscan bereit: ${selected.length} wirtschaftlich relevante Kandidaten aus ${withPrice.length} Sets. Der gebündelte Markt-Snapshot ersetzt tausende Einzelabfragen; unvollständige Topsets werden anschließend gezielt validiert.`
);
...
<h2>🎯 Badge Scout 3.0.0 – Schnellmarkt + Sammlungsradar + Marktampel</h2>
...
<br><br>
<b>3.0 Schnellmarkt:</b> Eine kleine Zahl globaler Steam-Marktseiten liefert viele günstige Karten gebündelt.
...
<br><br>
<b>SCE-Richtwert</b> ist nur ein externer Vorfilter, keine neue Bewertungsskala. Grün/MARKT-SUPER gibt es weiterhin erst mit ausreichend vollständigen Steam-Livepreisen und Sell-Supply.
<br><br>
...
<button id="validateMarket" class="btnGray" type="button">📊 Top-Markt exakt prüfen</button>
...
<span class="auto-more">⚡ zweistufig · 429-sicher · kein Scrollzwang</span>
</div>
</div>`;
...
freshHuntFilter.addEventListener('change',()=>{
...
S.cardNodes=new Map();
syncVisibleResults(true);
if(freshHuntFilter.checked&&S.activeBuyOrdersStatus!=='ready'){
void ensureActiveBuyOrders(false);
}
if(freshHuntFilter.checked&&!S.badgeCollectionLoadedAt){
