...
// @version      3.2.0
// @description  Portfolio-Wächter mit Current-/Future-Craft-Reserve, Badge-Zielen, konkurrenzbereinigter Kaufpreisprüfung sowie aktiven, gehaltenen und unbestätigten Verkaufsangeboten. Prüft Buy-/Sell-Depth, Historien, geduldige Preisziele und taktisches Verkaufen/Rückkaufen. Eigenständig; Shared-Intel optional. Keine automatischen Käufe/Stornos/Verkäufe.
...
let match=text.match(/\b([A-Z][a-z]{2})\s+(\d{1,2})(?:,?\s+(\d{4}))?/);
if(!match)match=text.match(/\b(\d{1,2})\s+([A-Z][a-z]{2})(?:\s+(\d{4}))?/);
if(!match)return null;
let month,day,year;
if(monthMap[match[1]]!==undefined){month=monthMap[match[1]];day=Number(match[2]);year=Number(match[3]||now.getFullYear());}
else{day=Number(match[1]);month=monthMap[match[2]];year=Number(match[3]||now.getFullYear());}
if(month===undefined||!Number.isFinite(day)||!Number.isFinite(year))return null;
let ts=Date.UTC(year,month,day,12,0,0);
if(!match[3] && ts>Date.now()+2*86400000)ts=Date.UTC(year-1,month,day,12,0,0);
return ts;
}
...
function sellListingStateInfo(row,listedDateText=''){
const nearby=[row];
for(const sibling of[row?.previousElementSibling,row?.nextElementSibling]){
if(sibling&&!/^mylisting_/i.test(String(sibling.id||''))&&clean(sibling.textContent).length<=600)nearby.push(sibling);
}
const statusText=clean(nearby.map(node=>node?.textContent||'').join(' '));
const structuralText=clean([row?.id,row?.className,row?.parentElement?.id,row?.parentElement?.className].join(' '));
const stateHaystack=`${statusText} ${structuralText}`;
const awaitingConfirmation=/(?:awaiting|pending|needs?)\s+(?:mobile\s+)?confirmation|confirm\s+(?:this\s+)?listing|best[aä]tigung\s+ausstehend|muss\s+best[aä]tigt|angebot\s+best[aä]tigen/i.test(stateHaystack);
const marketHold=/market[_\s-]*hold|on\s+hold|held\s+(?:by\s+steam\s+)?until|will\s+be\s+held|being\s+held|hold\s+for\s+\d+|available\s+(?:on|after)|will\s+be\s+(?:listed|available)|zur[uü]ckgehalten|haltefrist|verf[uü]gbar\s+ab/i.test(stateHaystack);
const listingState=awaitingConfirmation?'AWAITING_CONFIRMATION':marketHold?'MARKET_HOLD':'ACTIVE';
const holdTimestamp=marketHold?parseFutureMarketDate(statusText)||parseFutureMarketDate(listedDateText):null;
return{
listingState,
...
statusText:listingState==='ACTIVE'?'':statusText.slice(0,600)
};
}
...
function parseSellListingRows(html,assets=null,hovers=''){
const doc=new DOMParser().parseFromString(String(html||''),'text/html');
...
const buyerPayCents=moneyToCents(titleSpans[0]?.textContent||row.querySelector('.market_listing_price')?.textContent||'');
const sellerReceivesCents=moneyToCents(titleSpans[1]?.textContent||'');
const listedDateText=clean(row.querySelector('.market_listing_listed_date')?.textContent||'');
const listingStateInfo=sellListingStateInfo(row,listedDateText);
...
const parsed=parseSellListingRows(data.results_html||'',data.assets||null,data.hovers||'');
all.push(...parsed);
...
const byId=new Map();
for(const listing of all)byId.set(listing.listingId,listing);
return[...byId.values()];
}
...
overlay.innerHTML=`
<div class="sbw-modal">
<h2>🧹 Portfolio-Wächter 3.2: Kaufen + Badges + Verkaufsstatus</h2>
...
STATE.running=true;STATE.stopRequested=false;STATE.generated=null;STATE.groups=[];STATE.ownedCardApps=new Map();STATE.sellListings=[];STATE.steamId64=null;STATE.discoveryWarning=null;STATE.sellListingWarning=null;resetStats();createModal();
try{
...
'v3.2-own-order-adjusted-buy-plus-demand-aware-sell-hold-portfolio',

marketRiskVersion:
'v3.2-buy-competition-sell-demand-listing-state-reserves-net-rebuy',
